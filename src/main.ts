...
namespace Content {
    ...
    export function createSolidHtmlContent({ rootDir, filename, addMissingFavicon, replace, keepmaps }: Options): string {
        // 1. Parse HTML
...
function processSingleHtml(filename: string): void {
    ...
    const newCnt = Content.createSolidHtmlContent({
        rootDir,
        filename,
        addMissingFavicon: options.favicon,
        replace: options.replace,
        keepmaps: options.keepmaps
    });
...
export function main() {
    ...
    options.favicon = !args.nofav;
    options.keepmaps = args.keepmaps;

...
function createSolidHtmlContent(rootDir: string, htmlAsStr: string, addMissingFavicon: boolean): string {
    // 1. Parse HTML
    ...
    console.log(chalk.green(`Total link(s) ${files.length}:`));
...
function createSolidHtml(filename: string): void {
    ...
    let newCnt = createSolidHtmlContent(rootDir, htmlAsStr, ADD_MISSING_FAVICON);
